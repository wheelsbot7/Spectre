<script>
  ...
  import debianData from "/data/debian.json";
  import fedoraData from "/data/fedora.json";
  ...
  let debianCategories = debianData.map((item) => {
    let categoryName = Object.keys(item)[0];
    let programs = item[categoryName];
    return { categoryName, programs };
  });

  let fedoraCategories = fedoraData.map((item) => {
    let categoryName = Object.keys(item)[0];
    let programs = item[categoryName];
    return { categoryName, programs };
  });
  function join(packages) {
    switch (distro) {
      ...
      case "pacman":
        return "sudo pacman -Sy " + `${packages.join(" ")}`;
      default:
...
{#if distro == "apt"}
  <div class="container">
    {#each debianCategories as category}
      {#if Object.keys(category.programs).length > 0}
        <div class="category">
          <h3>
            {category.categoryName}
...
{#if distro == "dnf"}
  <div class="container">
    {#each fedoraCategories as category}
      {#if Object.keys(category.programs).length > 0}
        <div class="category">
          <h3>
            {category.categoryName}
